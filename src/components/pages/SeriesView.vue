      ...
      <div class="info-container">
        <div class="info">
          ...
          <div class="right">
            ...
            <ul class="genres">
              <li class="genre" v-for="(genre, index) in showData.genre" v-bind:key="index">{{ genre }}</li>
            </ul>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async loadInfo() {
      const { data } = await axios.get(`/series/${this.$route.params.seriesId}/info`);
    ...
    async loadEpisodes() {
      const { data } = await axios.get(`/series/${this.$route.params.seriesId}/episodes`);
      ...
      unsorted.forEach((v, i) => {
        v.sort((a, b) => a.airedEpisodeNumber - b.airedEpisodeNumber);

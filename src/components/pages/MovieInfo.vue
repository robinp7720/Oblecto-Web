      ...
      <div ref="infoContainer"
        ...
        <div class="info" v-if="posterLoaded">
          ...
          <div class="right">
            ...
            <ul class="genres">
              <li class="genre" v-for="(genre, index) in movieData.genre" v-bind:key="index">{{ genre }}</li>
            </ul>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    gradientColor() {
      const infoBottom = this.$refs.infoContainer.getBoundingClientRect().bottom;

      const wh = window.innerHeight;
      const st = (wh * 0.36);

      let w1 = infoBottom / st;

...
export default {
  ...
  methods: {
    async search(query) {
      const { data: episodes } = await this.axios.get(`/episodes/search/${query}`);
      const { data: movies } = await this.axios.get(`/movies/search/${query}`);
      const { data: series } = await this.axios.get(`/shows/search/${query}`);
  ...
  watch: {
    async $route(to) {
      await this.search(to.params.search);
    },
  },
